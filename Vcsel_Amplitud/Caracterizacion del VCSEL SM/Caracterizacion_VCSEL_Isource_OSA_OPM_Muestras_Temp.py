...
temp_end = 20                   #Temperatura inicial del Laser  [°C]
temp_step= 0.5                  #Paso de temperatura del Laser [°C]
...
level_ref = -3              #Nivel de referencia de potencia [dBm]
...
def Config_OSA_Yokogawa(Instrumento,Display_Status,Wavelength_Center,Wavelength_Span,Osa_Resolution,OSA_Sensitivity,Level_Offset,Level_Ref,Sweep_Mode,Sweep_Speed):
    ...
    OSA.write(":SENSE:SETTING:FCONNECTOR "+Sweep_Speed)                 #Establece el tipo de fibra
...
def VCSEL_Current_Sweep_Fix(Recurso_Isource,Min_Current,Max_Current,Step_Current,Recurso_OSA,Recurso_OPM):
...
###### Mediciones 
for j in np.arange(temp_start,temp_end,temp_step):   #Barrido de temperatura

                
    try:          
        os.mkdir('Temp_'+str(j))      
    except OSError as e:        
        if e.errno != errno.EEXIST:               
            raise
            

    for i in np.arange(1,Repeticiones,1):   #Barrido de muestras
        
        Basic_Temp_Set(Isource,j,temp_tol)
        Basic_Temp_Set(Isource,j,temp_tol)

        Tiempo,Corriente,Corriente_Barrido,Longitud_Onda,Potencia=VCSEL_Current_Sweep_Fix(Isource,current_min,current_max,current_step,OSA,OPM) #Barrido de corriente

        ####################### Data to .csv ##########################
        filename = "Temp_"+str(j)+"\VCSEL_1550_SM_Caracterizacion_"+dt_string+"_Temp_"+str(j)+"_Current_Step_"+str(current_step)+"_OPM_OSA_"+str(i)+".csv"
